...
class AuthControllerMvcTest {
    ...
    private final ContentmunchUser contentmunchUser = ContentmunchUser.builder().username("user").password("{noop}pass}")
            .email("user@email.com").name("User Name").roles(Set.of(ContentmunchRole.ROLE_USER, ContentmunchRole.ROLE_ADMIN))
            .build();
    ...
    void logout_shouldClearAuthCookie() throws Exception {
        ...
        mockMvc.perform(post("/api/auth/logout")).andExpect(status().isOk()).andExpect(content().string("Logged out"))
                .andExpect(
                        header().string(HttpHeaders.SET_COOKIE, Matchers.allOf(Matchers.containsString("contentmunch-auth="),
